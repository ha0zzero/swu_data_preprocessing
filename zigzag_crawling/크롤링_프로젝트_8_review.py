...
for id in range (1,31):
    ...
    for top_review in review_area:
        ID = id
        리뷰어 = top_review.select_one('div.BODY_16.SEMIBOLD.css-1k3hx0v.e1fnwskn0').text
        리뷰날짜 = top_review.select_one('span.BODY_17.REGULAR.BODY_13.MEDIUM.css-1w6topb.e1cn5bmz0').text
        리뷰텍스트 = top_review.select_one('div.BODY_14.REGULAR.css-epr5m6.e1j2jqj72').text
        review_list = [ID, 리뷰어, 리뷰날짜, 리뷰텍스트]
        result.append(review_list)
        if (len(result) == id * 5): break  # 상위 5개 리뷰 크롤링
    print('ID:' + str(id) + ' 리뷰 크롤링 완료')
    browser.back()
